...
const CreateCheckForm = ({ transactionTypes, settings }: Props) => {
  ...
  // Recalcula el feed cuando cambian amount o percent
  useEffect(() => {
  ...
  const onSubmit = async (data: CheckFormValues) => {
    try {
      const response = await fetch(
        "http://localhost:3000/api/checkTransaction",
        {
          method: "POST",
  ...
  const handleCustomerSelect = (customer: Customer) => {
    ...
    setValue("percent", customer.percentage, { shouldValidate: true });
    trigger("customerId");
  };
              ...
              <div className="space-y-3">
                ...
                <Select
                  onValueChange={(value) => {
                    ...
                    console.log(transactionTypes.find((t) => t.id === value) ?.name.toLowerCase())

                    setValue("percent", 
                      transactionTypes.find((t) => t.id === value) ?.name.toLowerCase() === "cash check"
                        ? customerFeed
                        : settings.numCustomerPercentRate,
                        { shouldValidate: true });
                    ...
                    setValue("feed", (amount * percent)/100, { shouldValidate: true });
